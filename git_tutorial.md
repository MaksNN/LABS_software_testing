...
### Для работы с Git в первую очередь необходимо установить и настроить. Для начала введем в cmd-консоль команду:
...
### После настройки Git необходимо инициализировать свой локальный репозиторий. Для этого переходим в нужное место в каталоге с помощью команды:
#### cd <адрес>
...
#### git add <имя_файла> (подготовить файл перед коммитом)
...
#### git commit -m <имя_коммита> (закоммитить файл)
...
#### git branch test_branch (создать новую ветку c именем test_branch, но не переключаться на нее)
### Теперь переключимся на нее командой:
#### git checkout test_branch (переключиться на ветку с именем test_branch)
